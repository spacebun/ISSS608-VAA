...
-   Homepage module: Users have an overview of the dataset used in application.
-   EDA and CDA module: Users can explore the distribution and trends in the dataset for specific periods or locations (station or region), and use statistical analysis to confirm the trends.
-   Univariate forecasting module: Users can explore the time series decomposition of the different variables, and select the type of model that they would like to use for univariate time series forecasting.
...
-   Geospatial module: It will include a map of Singapore and interactivity elements that allow users to view the dataset at different points in time.
...
pacman::p_load(tidyverse, naniar, imputeTS, DT, knitr, lubridate,
               ...
               tseries, tsibble, ggHoriPlot, feasts,
...
We will utilise Horizon Plots to visualize multiple time series data at a glance.
...
Convert data to tsibble format for analysis using **fable**:
...
selected_var <- "Minimum Temperature (°C)" # or "Daily Rainfall Total (mm)"
...
plot_ly(filtered_period_mstn, 
             ...
             text = ~paste("<b>Station:</b>", Station,
                           "<br><b>Date:</b>", Date,
                           "<br><b>", selected_var, ":</b>", filtered_period_mstn[[selected_var]])) %>%
  layout(title = paste(selected_var, "by Station"),
         xaxis = list(title = ""),
         yaxis = list(title = "")) %>%
...
Users should be able to first toggle between variables:
...
This module presents different methods to prepare an isohyet map by doing spatial interpolation for the data.

Steps:

-   Data preparation
-   Apply interpolation method
-   Visualize result with tmap
